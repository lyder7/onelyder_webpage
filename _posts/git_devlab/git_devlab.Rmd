---
title: "Git & Github: Branching"
description: |
  Bash scripting and commands.  
author:
  - name: Lyder Aleksander Mathisen 
    url: https://onelyder.com/about
#    affiliation: Me, Myself, and I
    affiliation_url: https://onelyder.com
output:
  distill::distill_article:
    css: small-spacing.css
    self_contained: false 
#preview: programming.jpg
---




## git kommandoer 

Terminal kommando for `git.` Under er nyttig kommandoer i forbindelse med `branching` av et prosjekt. Det vil si at det er en *developer* gren (branch) som parallelt med main (hoved) grenen av prosjektet foretar endringer til nye tilskudd/endringer.

```{bash eval=FALSE}
# Lager ny branch 
git branch from_startC dfed6bbc35053063a9b999d08170bb7b8c124504
# Åpne branch 
git checkout from_startC 
# print ut hvilken bracnh
git symbolic-ref HEAD
git add .
git commit -m "commit text"
# Push til branchen 
git push --set-upstream origin from_startC
```


```{bash eval=FALSE}
# Oversikt over commits
git log --graph 
#avslutt log modus 
shift + Q


```