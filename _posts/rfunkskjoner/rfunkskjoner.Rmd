...
Tekst, tall, en funksjoner etc. i en "system variabel" `x`.
Lagrer variabelen som en funksjon, og kan i prinsippet bestå av hva som helst.
...
Målet er å telle antall bokstaver i hvert navn i vektoren `navn`.\
God praksis er å gjøre klart objektet resultatene skal lagres i.
Her i eksempelet under er det antall bokstaver i hvert navn.
...
Videre lager vi en *loop*, den utfører operasjonen i en og samme "kode blokk".
Årsaken til at *loops** er nyttig her, er fordi man ønsker å unngå reputasjon. Loopen gjør oppgaven vi spesifiserer på alle elementene i vektoren. Vi trenger bare å skrive en "blokk" med kode. 
...
[^4]: Unngå missforståelser forbundet med "loops".
...
I dette kapittelet er formålet å bli bedre kjent med `purrr` og scenarioer hvor `purrr` kan være nyttig å bruke. Referanser til `tidy` og `purrr` se @wickham_r_2016 og @wickham_advanced_2019-4. 

`Purrr` er en pakke fra Tidyverse paradigme som hjelper til med funksjonelle operasjoner [@wickham_advanced_2019-4].

Prinsippene bak `tidy` filosofien og biblioteket `purrr` bistår oss med muligheter til å utføre operasjoner effektivt. Alternativet er å skrive egne funksjoner.

Risikoen med egne funksjoner er at den kan bli for komplisert og spesifikk for prosjektet/settingen funksjonen lages for. I praksis betyr det at funksjonen ikke kan videreføres til andre prosjekter eller problemstillinger. Over tid og ved gjenbruk på andre data sett kan den gi resultater som muligens er feil [@@wickham_advanced_2019-4; @hadleywhickham_advr_6functions].
...
Her er formålet å få et kjapt overblikk over hvordan data ser ut.
...
I de første kapitalene var vi innom det grunnleggende konseptet hvor man ved bruk av `<-` eller `=` kan "lagre" data i objektet som vi selv velger hva skal hete. `a <- 1` for eksempel.
Det vil si `a` representerer tallet `1`, refererer vi til objektet `a`, vil R tolke dette som `1`. 
...
Jeg tar derfor først for meg standardiserte funksjoner som er spesielle og egnet til å bruke i nye funksjoner også.

## rlang

For å gå litt dypere til verks for å forstå hva salgs hjelpemidler og funksjoner som er i pakkene jeg personlig bruker til funksjoner og funksjonell programmering i R.
I kodesnippen under tar jeg å henter ut funksjonsnavnene i pakken rlang, `magrittr`, og `purrr` [@mailund_functional_2019; @wickham_advanced_2019; @wickham_r_2016].
...
Printe for eksempel navnet til en funksjon, en operasjon eller en indeks som referanse.
