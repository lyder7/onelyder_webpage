...
Selv om `tidyverse` og alle dens fantastiske hjelpemidler til R brukere, er det alikevel etter min mening viktig å lære de fundamentale rammene.
Dette kalles "**base R**".

Funksjonene er standard og tilgjengelig for alle som laster ned R.
Det er ikke behov for å laste ned base R, slik man må gjøre med andre pakker.
For base R er kommandoene allerede tilgjengelig og kan brukes.
...
Muligheten til å se hvordan funksjonene er bygget opp er utrloig nyttig når man utvikler ferdighetene innenfor programmering.
Kilde koden gir muligens svaret man trenger for å konkretisere eller gi en klar definisjon på hvordan funksjonen/kommandoen fungerer.

[Link Base R](https://stat.ethz.ch/R-manual/R-devel/library/base/html/00Index.html)

Inne i R-studio, finn pakken `library`.
Åpne funksjonen.
...
Dette er notasjoner og syboler det er nyttig å lære.
...
NB!!
Husk at vi kan tweake funksjonen ved
...
Det er også mulig å gjøre flere essesnielle justeringer av funksjonen.
...
Hvis vi legger inn bare en liten endring og skriver `else if ()` gjør vi slik at programmet vurderer en ny logisk test før den utfører en spesiell type oppgave.
Dette er for å kontrollere funksjonens rute.
Ofte er det nødvendig med avklaringer, og dersom noe aviker (som det testest for i if utsagnene) vil programmet rettet det opp.

Dette kalles control flow innenfor funksjonell programmering.
Her er noen snipps som viser `else if` varianten:
...
NB else må vøre på samme linje som den siste brakken { for at R skal kunne avaluere den.
Hvis du skriver

[Logic checkpoints](https://stackoverflow.com/questions/25640161/r-how-to-test-for-character0-in-if-statement)

Bygge funksjoner er alfa-omega med koding.
Det å kunne lage- og bygge opp sine egne funksjoner er en superkraft.
I utgangspunktet kan man fint bruke funksjoner som allerede finnes i pakker.
Som regel er de aller fleste pakkene innenfor ulike fagfelt/emner store i omfang, slik at de dekker de fleste verktøyene man vil trenge for en analyse feks.

Men sannsynligheten for at et problem som ikke helt kan løses med pakken du bruker, er funksjonsbygging løsningen!

Konseptet om at det er mulig å gi et obejekt `x` `<-` `"hva_som_helst"`.
Altså, vi kan lagre tekst, tall eller funksjoner etc. inne i en sytste-variabel vi kaller `x`.
Det som gjør det kanskje enda mer forvirrende er at hvis vi lagrer variabelen som en funksjon, så kan den funksjonen i prinsippet bestå av hva som helst.
Det er dette som gjør det fantastisk.
...
Årsaken til at vi bruker `map_chr` er fordi outputen blir fremvist i et objekt lagret som "character" class.
...
Vi kan også lagre outputen og referere til den i etterkant hvis vi trenger det. Under gjør vi eksakt samme operasjon, utfører *class()* funksjonen til alle variablene i datasettet *data_name*.
...
Målet er å videre uvikle dem og sette de til verks.
...
Neste steg er å generalisere opperasjonene som skal gjøres excell arkene.
Første spørsmål: *er opperasjonen lik for alle data-sett og alle kolonner?*
