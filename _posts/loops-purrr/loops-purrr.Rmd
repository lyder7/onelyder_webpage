...
Printe feks navnet til en funksjon, operasjon eller indeks som referanse.
...
`tidyverse` og med alle data science verktøyee til R brukere, er det allikevel etter min mening viktig å lære de fundamentale rammene.
"**base R**".
...
Det er også mulig å gjøre flere essensielle justeringer av funksjonen.
...
Liten endring, skriver `else if ()` gjør vi slik at programmet vurderer en ny logisk test før den utfører en spesiell type oppgave.
Dette er for å kontrollere funksjonens rute.
Ofte er det nødvendig med av klaringer, og dersom noe avviker (som det testes for i `if` utsagnene) vil programmet rettet det opp.

Dette kalles *control flow*.
Noen chunks som viser `else if` varianten:
...
NB `else` må være på lik linje med den siste brakken `{` for at R skal kunne evaluere den.
...
Konseptet om at det er mulig å gi et objekt `x` `<-` `"hva_som_helst"`.
Altså, vi kan lagre tekst, tall eller funksjoner etc. inne i en sytstem-variabel vi kaller `x`.
